    ...
    if context.user_data.get("awaiting_num"):
        ...
        for j in range(0, len(selected), INVITE_BATCH):
            ...
            try:
                ...
                # server asks to wait
                await update.message.reply_text(f"⚠️ FloodWait: sleeping {e.seconds}s... (then retrying this batch)")
                await asyncio.sleep(e.seconds)
                # retry the same batch (no change to loop index)
                continue
            except PeerFloodError:
                await update.message.reply_text("❌ PeerFloodError: Telegram is limiting invites from this account. Stopping invites to avoid ban.")
                break
            except UserPrivacyRestrictedError as e:
                # some users can't be invited — skip them
